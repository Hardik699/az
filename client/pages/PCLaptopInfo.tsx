  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-deep-900 via-blue-deep-800 to-slate-900">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <Card className="bg-slate-900/50 border-slate-700 backdrop-blur-sm">
          <CardHeader>
            <CardTitle className="text-white">Available IT Hardware</CardTitle>
          </CardHeader>
          <CardContent>
            {(() => {
              const sysRaw = localStorage.getItem(SYS_STORAGE_KEY);
              ...
              const usedKeyboardIds = getUsedIds(items, "keyboardId");
              const usedMotherboardIds = getUsedIds(items, "motherboardId");
              const usedCameraIds = getUsedIds(items, "cameraId");
              const usedHeadphoneIds = getUsedIds(items, "headphoneId");
              const usedPowerSupplyIds = getUsedIds(items, "powerSupplyId");
              const usedStorageIds = getUsedIds(items as any, "storageId" as any);
              const usedRamIds = Array.from(
                new Set([
                  ...getUsedIds(items, "ramId"),
                  ...getUsedIds(items as any, "ramId2" as any),
                ])
              );
              ...
              const availableAssets = allAssets.filter(
                (asset) => !allUsedIds.has(asset.id)
              );

              if (allAssets.length === 0) {
                return <p className="text-slate-300">No IT hardware items added yet</p>;
              }

              // Group by category
              const categories: { [key: string]: SysAsset[] } = {};
              ...
              return (
                <div className="space-y-4">
                  {Object.entries(categories).length === 0 ? (
                    <p className="text-slate-400">All IT hardware items are already assigned to PCs/Laptops</p>
                  ) : (
                    Object.entries(categories).map(([category, assets]) => (
                      <div key={category} className="border border-slate-700 rounded-lg p-4">
                        <h3 className="text-slate-300 font-semibold mb-3">
                          {categoryLabels[category] || category} ({assets.length})
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                          {assets.map((asset) => (
                            ...
                            >
                              <p className="text-cyan-400 font-mono">{asset.id}</p>
                              {category === "ram" && (asset as any).ramSize && (
                                <p className="text-slate-400 text-xs mt-1">
                                  {(asset as any).ramSize}
                                </p>
                              )}
                              {category === "storage" && (
                                <>
                                  {(asset as any).storageType && (
                                    <p className="text-slate-400 text-xs mt-1">
                                      Type: {(asset as any).storageType}
                                    </p>
                                  )}
                                  {(asset as any).storageCapacity && (
                                    <p className="text-slate-400 text-xs">
                                      Capacity: {(asset as any).storageCapacity}
                                    </p>
                                  )}
                                </>
                              )}
                              {category === "motherboard" && (asset as any).processorModel && (
                                <p className="text-slate-400 text-xs mt-1">
                                  {(asset as any).processorModel}
                                </p>
                              )}
                            </div>
                          ))}
